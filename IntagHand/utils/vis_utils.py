...
class mano_two_hands_renderer(Renderer):
    ...
    def render_rgb(self, cameras=None, scale=None, trans2d=None,
        ...
        # OUTPUT OBJ FILES
        facess = self.faces.repeat(bs, 1, 1)
        left_faces = torch.from_numpy(self.mano['left'].get_faces().astype(np.int64)).to(self.device).unsqueeze(0)
        right_faces = torch.from_numpy(self.mano['right'].get_faces().astype(np.int64)).to(self.device).unsqueeze(0)
        left_faces = right_faces[..., [1, 0, 2]]

        outputobjl = '/home/kkoshi/output_left'+str(self.ind)+'.obj'
        outputobjr = '/home/kkoshi/output_right'+str(self.ind)+'.obj'
        outputobj = '/home/kkoshi/output_together'+str(self.ind)+'.obj'

        save_obj(outputobjl,v3d_left[0].detach(),left_faces[0].detach())
        save_obj(outputobjr,v3d_right[0].detach(),right_faces[0].detach())
        save_obj(outputobj,v3d[0].detach(),facess[0].detach())
        self.ind+= 1
        ####
